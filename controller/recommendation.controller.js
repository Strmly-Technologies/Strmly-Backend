...
const checkAccess = async (video, userId) => {
  try {
    if (video.type === 'Free') {
      ...
      if (video.series) {
        console.log("Entered series for", video.name);
        const series = await Series.findOne({ episodes: video._id }).populate('episodes');
        const hasSeriesAccess = await UserAccess.findOne({
          user_id: userId,
        ...
        if (hasSeriesAccess) {
          video.access = {
            ...
            isPurchased: true,
            accessType: 'series_access',
          };
        }
        else {
          console.log("No series access for", video.name);
          video.access = {
            isPlayable: false,
            freeRange: {
              start_time: video.start_time || 0,
            ...
            isPurchased: false,
            accessType: 'limited_series',
            price: video.amount || 0,
          };
        }
        console.log(video.access);
        return video;
      }

...
const searchVideos = async (req, res, next) => {
  try {
    ...
    const [longVideos, shortVideos] = await Promise.all([
      LongVideo.find({
        $or: [{ name: searchRegex }, { description: searchRegex }, { genre: searchRegex }],
      })
...
const getVideoById = async (req, res, next) => {
  try {
    ...
    let video;
    if (type === "short") {
      video = await ShortVideo.findById(id).populate("created_by", "username email").populate("community", "name");
    } else {
      video = await LongVideo.findById(id).populate("created_by", "username email").populate("community", "name").populate("series", "title");
    }
...
const updateVideo = async (req, res, next) => {
  try {
    ...
    if (type === "short") {
      ...
      video = await ShortVideo.findByIdAndUpdate(id, updateData, { new: true });
    } else {
      ...
      video = await LongVideo.findByIdAndUpdate(id, updateData, { new: true });
    }
...
const getTrendingVideos = async (req, res, next) => {
  try {
    ...
    if (type === "short") {
      ...
      total = await Promise.all([LongVideo.countDocuments(), ShortVideo.countDocuments()]);
    }
    ...
    res.status(200).json({
      ...
      pagination: {
        currentPage: parseInt(page),
        totalPages: type ? Math.ceil(total / limit) : Math.ceil((total[0] + total[1]) / limit),
...
const incrementVideoView = async (req, res, next) => {
  try {
    ...
    let video;
    if (type === "short") {
      video = await ShortVideo.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true });
    } else {
      video = await LongVideo.findByIdAndUpdate(id, { $inc: { views: 1 } }, { new: true });
    }
